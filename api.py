        ...
        if type(x) is dict:
            ...
            if x != "":
                ...
                if "table_" in name:
                    ...
                    table_key = (
                        "table_" + str(table_ctr) + "_" + name.split("_table_")[-1]
                    )
                    try:
...
async def async_prepare_index(loop):
    ...
    while not stop_flag and i < 5:
        tasks.append(
            (
                loop.create_task(
                    get_page("https://sarkariyojana.com/karnataka/page/" + str(i))
                ),
                i,
            )
        )
...
def return_content():
    ...
    try:
        return json.jsonify({"desc": scheme_desc, "image": scheme_img})
    except Exception as e:
        return str(repr(e))
