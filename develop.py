...
class SCHEME:
    ...
    async def get_page(url, get_blob=False):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                # async with session.get("127.0.0.1:5000") as resp:
...
def send_content():
    try:
        ...
        return json.jsonify(SCHEME.LIST[int(schemeid)].content)  # c.OrderedDict(scheme_content[int(i)])#scheme_content[int(i)]
    except Exception as e:
        abort(400)
